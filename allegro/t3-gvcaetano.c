...
// gcc -Wall -o t3-gvcaetano t3-gvcaetano.c tela.c -lallegro_font -lallegro_color -lallegro_ttf -lallegro_primitives -lallegro
...
void jogo(int *vidas, int *pontos, int *ini_atv)
{
  ...
  caixa cursor = {0, A_TELA - 30, 200, 20, azul, branco};
  while (*vidas) {
    controla_barra(&cursor);
    ativa_bola(ini_atv, inimigo);
    for (int i=0;i<QNT_MAX;i++) {
      if (inimigo[i].esta == ATIVO || inimigo[i].esta == CAIU) {
        rebate_na_borda(&inimigo[i]);
        inimigo[i].x += inimigo[i].vx;
        inimigo[i].y += inimigo[i].vy;
        inimigo[i].vy += XLR8;
        if (inimigo[i].esta == ATIVO) {
          rebate_ou_morre(&inimigo[i], &cursor, pontos, ini_atv, vidas);
        }
      }
      if (inimigo[i].esta == INAT) {
        pisca(&inimigo[i]);
        if (inimigo[i].timer + 3 <= tela_relogio() &&
            (int)(tela_relogio()) % 3 == i) {
          inimigo[i].esta = ATIVO;
          inimigo[i].cor = vermelho;
          inimigo[i].borda = branco;
          *ini_atv += 1;
      ...
      if (inimigo[i].esta == CAIU && tela_relogio() - inimigo[i].timer >= 1.2) {
        inimigo[i].esta = INAT;
        inimigo[i].y = H_MAX;
        inimigo[i].vy = 0;
        inimigo[i].timer = tela_relogio();
  ...
  if (limite_inferior && dentro_da_raquete) {
    ...
    *pontos += 1;
    if (bola->vx < 10 && bola->vx > -10) {
      bola->vx += (bola->vx + (bola->x - caixa->xc)) / 5;
    }
